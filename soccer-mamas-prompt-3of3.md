...
- **MANDATORY STATE PURGE before flipping:** Before swapping positions, zero out ALL player velocities (`vx = 0, vy = 0`), clear ALL `burstTarget` values, clear ALL `burstCooldownUntil` timers, clear ALL `cooldownUntil` (dispossession) timers, reset ALL `lockoutTimer` values to 0, clear `ball.lastCarrierId`, and reset `ball.checkedBy`. Without this, players will attempt to run toward their old-side targets after the flip, or a player tackled at 44:59 could start the second half locked out.
